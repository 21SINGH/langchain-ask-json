    ...
    if json_file is not None:
        try:
            ...
            user_question = st.text_input(
                "Ask a question about your JSON:",
                value="Give me 1 product that will work well with moisturiser",
            )
            if user_question:
                ...
                st.write(f"Selected chunk for LLM: {selected_doc.page_content}")
                ...
                prompt_template = """The user has added {context} to the cart. Based on the product catalog and typical skincare routines, recommend 1 additional products that are essential to pair with it. Start by briefly explaining why they’re needed (the use case), then suggest the products by name with a short benefit for each. Keep the tone helpful, not push. give response in Less than 90 character 
                Context: {context}
                Question: {question}"""
                ...
                with get_openai_callback() as cb:
                    response = qa_chain.invoke(
                        {"query": user_question, "context": selected_doc.page_content}
                    )
                    print(cb)
